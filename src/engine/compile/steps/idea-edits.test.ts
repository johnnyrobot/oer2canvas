...
test('a replace edit changes the nth occurrence and preserves case', () => {
  const edits = new Map<string, IdeaEdit>([
    [ideaEditKey('s1', b0, 1, 'Suffers from'), { kind: 'replace', replacement: 'has' }],
  ])
  const { html } = compile('<p>Suffers from one. Suffers from two.</p>', edits)
  expect(html).toContain(`<p id="${b0}">Suffers from one. Has two.</p>`)
})

test('a keep edit with context inserts a parenthetical after the original', () => {
  const edits = new Map<string, IdeaEdit>([
    [ideaEditKey('s1', b0, 0, 'schizophrenics'), { kind: 'keep', context: 'a term used at the time' }],
  ])
...
test('a keep edit without context changes nothing and adds no change note', () => {
  const edits = new Map<string, IdeaEdit>([[ideaEditKey('s1', b0, 0, 'x'), { kind: 'keep' }]])
...
test('an edit whose original is no longer there is dropped with a note, never applied', () => {
  const edits = new Map<string, IdeaEdit>([
    [ideaEditKey('s1', b0, 0, 'gone'), { kind: 'replace', replacement: 'x' }],
  ])
  const { html, notes } = compile('<p>nothing to match</p>', edits)
  expect(html).toContain(`<p id="${b0}">nothing to match</p>`)
...
test('an edit for another section is ignored', () => {
  const edits = new Map<string, IdeaEdit>([
    [ideaEditKey('s2', blockId('s2', 0), 0, 'crazy'), { kind: 'replace', replacement: 'wild' }],
  ])
  expect(compile('<p>crazy</p>', edits).html).toContain(`<p id="${b0}">crazy</p>`)
})

test('the change note is appended to the attribution block once, when something applied', () => {
  const edits = new Map<string, IdeaEdit>([
    [ideaEditKey('s1', b0, 0, 'crazy'), { kind: 'replace', replacement: 'wild' }],
    [ideaEditKey('s1', b1, 0, 'crazy'), { kind: 'replace', replacement: 'wild' }],
  ])
...
test('with no edits the step is a no-op', () => {
  ...
  expect(html).toContain(`<p id="${b0}">crazy</p>`)
